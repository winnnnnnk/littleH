...
xiaoh是Codex根对话中的研发协调者。你可以直接说目标、现象或想法，xiaoh负责核对事实、提出推荐方案、协调专业Agent、验证结果，并把已经验收的内容写入长期知识库。

xiaoh不会把疑问当成需求变更，也不会因为用户的一句反问就删除范围。用户说法与代码、配置、数据或运行证据不一致时，xiaoh会说明冲突和影响，再给出推荐结论。

仓库只包含通用能力，不保存客户资料、具体项目、真实任务证据、账号或凭据。

## 先看哪份文档

- [认识xiaoh](docs/xiaoh-guide.md)：适合第一次使用，说明怎么沟通、任务怎么推进、什么时候需要人工决定。
- [xiaoh实现设计](docs/implementation-design.md)：面向维护者，说明组件、门禁、Agent体系和Obsidian结构。
- [xiaoh与Playbook的关系](docs/xiaoh-playbook-relationship.md)：说明独立模式和Playbook受管模式如何分工。
...
两端都需要Git、Python 3和Codex CLI。Obsidian不是初始化的硬依赖；如果要使用xiaoh的个人研发系统，建议安装Obsidian 1.9或更高版本。
...
源码安装器会注册当前目录为本地Marketplace、安装`xiaoh`插件并部署运行时。Codex定时任务只能通过应用提供的任务工具创建，所以命令行安装可能暂时显示`degraded`。重新打开Codex并完成初始化后，xiaoh会读取真实任务状态并完成绑定。
...
xiaoh不是子Agent，插件不会创建`agents/xiaoh.toml`。

## 可选能力

依赖事实源是`plugins/xiaoh/dependencies.json`。

Ponytail用于已确认方案的代码实现阶段。浏览器、文档、PDF、表格、演示、站点和可视化插件按需安装。`codebase-memory-mcp`可以提供代码知识图谱，缺失时xiaoh改用本地搜索。

Playbook是可选的受管研发平台。只有任务明确由Playbook管理时，xiaoh才启用适配器。机器上存在`playbook`命令不会改变普通任务的流程。

Playbook CLI的安装、升级、降级、重装、版本切换和安装源切换由用户在Codex外部终端人工完成。xiaoh只读检查版本、路径、包来源和兼容性，不会替用户修改Playbook。
...
任务上下文、需求门禁和本地评审清单由`validate.py`验证。完整命令与发布检查见[xiaoh实现设计](docs/implementation-design.md#20-验证入口)和[安全与可信边界](SECURITY.md)。
