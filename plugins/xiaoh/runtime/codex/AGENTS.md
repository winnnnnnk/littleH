...
- 实现前记录基线和写入范围，保持单一写入责任；实现后核对实际 diff 和越界变化。
- 根据风险执行格式检查、静态分析、单元测试、集成测试、构建、契约测试或安全测试。
- 测试失败时定位根因并修复；不要把“再次评审”当作默认修复机制。
- 任务收口要求 schema 1.2 运行记录中的全部 gate 和 verification 为 `passed`，且 `metrics.result_accepted=true`。
