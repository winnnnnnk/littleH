...
- 用户统一与xiaoh沟通，xiaoh负责需求理解、上下文选择、任务拆分、调度、冲突裁决、验收和汇报。
...
- 全局 `AGENTS.md` 是根线程xiaoh的执行事实源；各 Agent TOML 只定义可委派的专业子 Agent；本页是供人阅读的目录与调度说明。
...
- 高风险设计或实现必须由未承担该实现的适用判断角色独立评审，xiaoh负责综合但不替代专业判断。
...
| 任务类型 | 默认角色组合 |
|---|---|
| 需求/领域分析 | xiaoh + PKI 领域专家；复杂系统增加 Java 架构专家 |
| 单仓 Java 功能 | xiaoh + 代码探索 + Java 实现 + 测试验证 |
| 跨仓改造 | xiaoh + Java 架构 + 代码探索；实现阶段按独立 repo 分派实现 Agent |
| PKI 安全敏感变更 | 常规组合 + PKI 安全评审 |
| 前后端联动 | xiaoh + Java 架构 + Java 实现 + 前端实现，分 repo 写入 |
| 交付前评审 | xiaoh + 代码质量评审 + 测试验证；安全敏感时增加 PKI 安全评审 |
...
- 临时 Agent 必须有明确输入、允许范围、输出格式和结束条件；结果经xiaoh验收后，仅把长期有效的确认事实写入上下文。

## Playbook 关系

- 全局 Agent 定义可复用能力，不复制具体 workspace 的状态机和命令。
- 进入 Playbook 受管环境后，xiaoh按 workspace 规则、runtime/worker 返回和 task state 调度角色。
- 专业 Agent 是叶子执行者、思考者或判断者，不绕过 Playbook 推进任务状态和交付生命周期。

## 可执行契约

- xiaoh使用 `__CODEX_HOME__/agent-system/task-context.template.json` 形成任务级上下文包；Playbook 受管任务以 worker/task 返回为状态事实源，模板不覆盖其内容。
