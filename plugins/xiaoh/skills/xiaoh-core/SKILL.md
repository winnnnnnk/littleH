...
- For a Playbook-managed business task, require `integrations.playbook` to resolve to `auto` or `enabled`, then run `$xiaoh:xiaoh-playbook-adapter` after obtaining the current worker JSON and immediately before formal specialist delegation. Bind the XiaoH Workspace identity separately from the Playbook task Workspace identity, map the delegated role to one action, and stop if the integration is disabled or the read-only adapter receipt is missing, stale, inconsistent, or incompatible.
