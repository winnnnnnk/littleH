...
5. Prefer updating an existing canonical page over creating a synonym. When several candidates describe one capability, merge only non-conflicting evidence and preserve every source link.
6. Write formal frontmatter with `type`, `knowledge_scope`, `knowledge_state: formal`, `status: accepted`, `domain`, `owner: xiaoh`, `updated`, `source_project`, and `source_evidence` as applicable.
7. Use the matching structure:
   - project/domain knowledge: definition or conclusion, context, confirmed rules, decision reasons, boundaries, verification, sources, unknowns;
...
8. Link project knowledge from the corresponding project homepage or knowledge index. Do not copy raw logs into formal knowledge.
9. Update the candidate projection to `knowledge_state: promoted`, `status: accepted`, add `promoted_to`, promotion date, scope, and evidence summary. Keep the authoritative closeout record unchanged.
10. If promotion would change Agent permissions, governance gates, Skill behavior, security rules, or an accepted business result, stop at a promotion recommendation and obtain the required user decision first.
11. Report the configured Vault root, exact written files, selected scope and why the other scopes were rejected, evidence basis, remaining uncertainty, and whether a Skill or Agent improvement candidate was created.
