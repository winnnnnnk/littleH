...
- 增加严格受管的原子根执行 bootstrap，解除 task-context 与首次 binding 的循环依赖；仿冒路径、命令拼接、额外参数和会话漂移继续失败关闭。
- `WorkspaceService.register` 自动把旧单路径 `roots` 列表迁移为 `darwin` 映射，并明确拒绝歧义列表。
- 安装清单升级到 v3，持久化文件级受管 inventory；退出 inventory 的旧文件纳入 Plan、备份、删除、写入记录和失败补偿。
- `guard_task_writes.py` 与 `xiaoh_security/execution.py` 继续作为 4.0.1 受管文件，并实现新契约，而非遗留 4.0.0 实现。
- 保持 4.0.0 产品能力、公共命令和用户长期资产兼容，不自动安装到本机。
